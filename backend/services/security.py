...
DEFAULT_AUDIT_LOG = os.path.join(tempfile.gettempdir(), "prepai", "audit.log")
AUDIT_PATH = Path(os.getenv("PREPAI_AUDIT_LOG", DEFAULT_AUDIT_LOG))
...
def prune_stale_buckets(buckets: dict[str, list[float]], window_sec: float) -> None:
    now = time.time()
    stale_keys = [
        k for k, timestamps in buckets.items() if not timestamps or (now - timestamps[-1] >= window_sec)
    ]
    for k in stale_keys:
        buckets.pop(k, None)
...
def audit_event(event: str, request: Request, extra: dict | None = None) -> None:
    ...
    if extra:
        ...
        safe = {
            ...
            if k.lower() not in {"password", "token", "authorization", "api_key"}
        }
        record.update(safe)
